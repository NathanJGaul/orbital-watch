...
public class SeedService(IServiceProvider services, ILogger<SeedService> logger) : IHostedService
{
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        var dbContext = scope.ServiceProvider.GetRequiredService<OrbitalWatchDbContext>();

        await ApplyPendingMigrationsAsync(dbContext, cancellationToken);

        if (await dbContext.Satellites.AnyAsync(cancellationToken))
        {
        ...
        dbContext.Satellites.AddRange(satellites);
        await dbContext.SaveChangesAsync(cancellationToken);
        logger.LogInformation("Seeded {Count} satellites.", satellites.Count);
    }

    private static Task ApplyPendingMigrationsAsync(
        OrbitalWatchDbContext dbContext,
